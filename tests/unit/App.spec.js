...
describe('Counter', () => {
    ...
    const createComponent = (props) => {
        wrapper = shallowMount(App, {
            ...
            propsData: props,
            stubs: {
                // CounterInput: false - чтобы рендерился настоящий компонент
    ...
    it('updates current value when CounterInput provides new one', async () => {
        ...
        createComponent({initialValue: INITIAL_VALUE});
        wrapper.findComponent(CounterInput).vm.$emit(CounterInput?.model?.event ?? 'input', NEW_INITIAL_VALUE)
        await nextTick();
        expect(wrapper.text()).toContain(`${NEW_INITIAL_VALUE} / 0`)
    })
